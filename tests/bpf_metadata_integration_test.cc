...
// Test: Push a listener via LDS with cilium.bpf_metadata using ADS mode for npds_config.
TEST_P(BpfMetadataIntegrationTest, BpfMetadataWithNpdsNpdhsViaAds) {
  on_server_init_function_ = [&]() {
    ...
    EXPECT_TRUE(compareDiscoveryRequest(Config::TestTypeUrl::get().Cluster, "", {}, {}, {},
                                        /*expect_node=*/true, Grpc::Status::WellKnownGrpcStatus::Ok,
                                        "", ads_stream_.get()));
    sendCdsResponse("1");
